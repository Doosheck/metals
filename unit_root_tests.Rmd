...
calculate_returns <- function(prices) {
  returns <- apply(prices, 2, function(x) {
    x <- na.omit(x)
...
cat("Lithium return series:", ncol(lithium_returns), "\n")
cat("Nickel return series:", ncol(nickel_returns), "\n")
cat("Cobalt return series:", ncol(cobalt_returns), "\n")
cat("Copper return series:", ncol(copper_returns), "\n\n")
```
...
for(i in 1:ncol(lithium_returns)) {
  series_name <- colnames(lithium_returns)[i]
...
for(i in 1:ncol(nickel_returns)) {
  series_name <- colnames(nickel_returns)[i]
...
for(i in 1:ncol(cobalt_returns)) {
  series_name <- colnames(cobalt_returns)[i]
...
for(i in 1:ncol(copper_returns)) {
  series_name <- colnames(copper_returns)[i]
