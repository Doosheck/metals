...
# Wybieramy TYLKO te daty, które odpowiadaj¹ wynikom modelu
...
# 3. Tworzymy listê wszystkich par (10 kombinacji)
...
# 4. £¹czymy wszystko w jedn¹ d³ug¹ tabelê
df_plot <- bind_rows(lista_korelacji)

# 5. Rysujemy 10 wykresów w siatce (facet_wrap)
dcc_plot_NI <- ggplot(df_plot, aes(x = Date, y = Wartosc)) +
  ...
  labs(
    title = NULL, # Brak tytu³u g³ównego
    x = NULL, 
    y = ""
  ) +
  theme(
...
ggsave(filename = "graphsR/DCC_nickel.pdf", 
       plot = dcc_plot_NI, 
       width = 12, 
       height = 10)
...
dcc_plot_CU <- ggplot(df_plot, aes(x = Date, y = Wartosc)) +
  ...
  facet_wrap(~Para, ncol = 2, scales = "free_y") + # 2 kolumny, 5 wierszy
  scale_x_date(date_labels = "%Y", date_breaks = "2 years") + # Poprawne daty
  theme_minimal(base_size = 12) + # Mniejsza czcionka, ¿eby 10 wykresów siê zmieœci³o
  labs(
    title = NULL, # Brak tytu³u g³ównego
    x = NULL, 
    y = ""
...
ggsave(filename = "graphsR/DCC_copper.pdf", 
       plot = dcc_plot_CU, 
       width = 12, 
       height = 10)
...
par(mfrow = c(4, 2), mar = c(2, 2, 2, 1))
# ACF i PACF dla kwadratów zwrotów:
for (col_name in names(dcc_returns)[-1]) {
...
# 2. Powielenie specyfikacji dla 4 szeregów
...
# --- WYNIKI ---
print(fit) # Wyœwietla oszacowania GARCH (Etap 1) i DCC (Etap 2)
```
