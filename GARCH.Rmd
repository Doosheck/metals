...
copper_returns_matrix1 <- as.matrix(copper_returns_df[, -7])
copper_returns_matrix <- as.matrix(copper_returns_matrix1[, -1])
...
dcc_fit <- dccfit(dcc_spec, data = as.matrix(copper_returns_matrix * 100), 
...
dcc_plot_CU <- ggplot(df_plot, aes(x = Date, y = Wartosc)) +
  ...
  theme_minimal(base_size = 9) + # Mniejsza czcionka, ¿eby 10 wykresów siê zmieœci³o
  labs(
...
fit <- dccfit(dspec, data = as.matrix(dcc_returns[,-1]* 100))
