...
This script mirrors the structure/flow of lithium_data_extraction.py for copper.
...
def create_missing_data_heatmap(df, output_dir):
    ...
    try:
        ...
        ax.set_title('Missing Data Pattern in Copper Price Series', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        heatmap_file = output_dir / 'copper_missing_data_heatmap.png'
...
def create_smart_plots(df, output_dir):
    ...
    try:
        ...
        fig.suptitle('Copper Price Series (Missing Values Excluded)', fontsize=20, fontweight='bold')
        ...
        smart_subplot_file = plots_dir / 'copper_smart_subplots.png'
        ...
        ax.set_title('Copper Price Series Comparison (Missing Values Excluded)', 
        ...
        smart_combined_file = plots_dir / 'copper_smart_combined.png'
        ...
        ax.set_xlabel('Date', fontsize=14)
        ax.set_title('Data Availability Timeline for Copper Price Series', 
                    fontsize=16, fontweight='bold')
        ax.grid(True, alpha=0.3, axis='x')
        
        plt.tight_layout()
        availability_file = plots_dir / 'copper_data_availability.png'
        ...
        plt.tight_layout()
        summary_file = plots_dir / 'copper_missing_data_summary.png'
...
def create_synchronized_missing_value_plot(df, output_dir):
    ...
    try:
        ...
        sync_file = plots_dir / 'copper_synchronized_missing_indicators.png'
...
def extract_copper_data():
    ...
    # Primary consolidated source for copper (already harmonized across feeds)
    print("Loading consolidated copper data (ALL_copper_prices_outer.csv)...")
    dfcu = pd.read_csv('data/ALL_copper_prices_outer.csv')
    ...
    try:
        ...
        csv_to_json_mapping = {
            ...
            "Price_COMEX_CO": "Price_COMEX_CO",
            "Price_LME_CO_3M": "Price_LME_CO_3M",
        ...
        copper_ticker_mapping = {}
        for csv_col, json_key in csv_to_json_mapping.items():
            if json_key in copper_ticker_mapping_raw:
                copper_ticker_mapping[csv_col] = copper_ticker_mapping_raw[json_key]
        
        print(f"Loaded {len(copper_ticker_mapping)} copper ticker mappings")
        print(f"Mapped columns: {list(copper_ticker_mapping.keys())}")
    ...
    keep_cols = ['Date'] + [col for col in copper_ticker_mapping.keys() if col in dfcu.columns]
    existing_cols = [c for c in keep_cols if c in dfcu.columns]
    dfcu = dfcu[existing_cols]
    
    print(f"Available columns from mapping: {[col for col in copper_ticker_mapping.keys() if col in dfcu.columns]}")
    print(f"Missing columns from mapping: {[col for col in copper_ticker_mapping.keys() if col not in dfcu.columns]}")

    # Convert numeric columns to float, handling strings if needed
    for col in dfcu.columns:
        if col == 'Date':
            continue
        dfcu[col] = pd.to_numeric(dfcu[col], errors='coerce')

    # Sort and de-duplicate
    dfcu = dfcu.drop_duplicates(subset=['Date']).sort_values(by='Date')

    print(f"  Consolidated copper: {dfcu.shape[0]} rows, {dfcu['Date'].min()} to {dfcu['Date'].max()}")

    # Remove unwanted metadata columns if they slipped in
    unwanted_columns = ['Net', '%Chg', 'Open', 'Low', 'High', 'Volume', 'Bid', 'Ask']
    columns_to_remove = [col for col in unwanted_columns if col in dfcu.columns]
    if columns_to_remove:
        dfcu = dfcu.drop(columns=columns_to_remove)
        print(f"Removed unwanted columns: {columns_to_remove}")

    # Analyze missing data BEFORE interpolation
    missing_stats_before = analyze_missing_data(dfcu, "Before Interpolation")
    ...
    dfcu = dfcu.interpolate(method='linear', limit_direction='both')
    print("Interpolation completed.")

    print("Rounding price columns to 4 decimal places...")
    price_columns = [col for col in dfcu.columns if col != 'Date']
    dfcu[price_columns] = dfcu[price_columns].round(4)
    print("Rounding completed.")

    # Analyze missing data AFTER interpolation
    missing_stats_after = analyze_missing_data(dfcu, "After Interpolation")

    print(f"\nMerged data shape: {dfcu.shape}")
    print(f"Date range: {dfcu['Date'].min()} to {dfcu['Date'].max()}")
    print(f"Columns: {list(dfcu.columns)}")
    ...
    # Save base series
    out_series_file = output_dir / 'copper_prices_series.csv'
    dfcu.to_csv(out_series_file, index=False)
    print(f"\nCopper data saved to: {out_series_file}")

    # Save raw (non-interpolated) data as well
    out_raw_file = output_dir / 'copper_prices_series_raw.csv'
    dfcu_raw.to_csv(out_raw_file, index=False)
    print(f"Raw copper data (no interpolation) saved to: {out_raw_file}")

    # Create tickers mapping from loaded JSON
    # Only map columns that actually exist in our data
    ticker_mapping = {col: ticker for col, ticker in copper_ticker_mapping.items() 
                     if col in dfcu.columns}

    dfcu_tickers = dfcu.copy().rename(columns=ticker_mapping)
    out_tickers_file = output_dir / 'copper_prices_tickers.csv'
    dfcu_tickers.to_csv(out_tickers_file, index=False)
    print(f"Copper data with tickers saved to: {out_tickers_file}")

    print("\nColumn mapping:")
    for old_name, new_name in ticker_mapping.items():
        if old_name in dfcu.columns:
            print(f"  {old_name} → {new_name}")

    # Enhanced plotting section
    print("\nCreating enhanced copper price plots...")
    try:
        import matplotlib.pyplot as plt
        import seaborn as sns
        ...
        create_missing_data_heatmap(dfcu_raw, plots_dir)
        
        # Create smart plots that handle missing values
        create_smart_plots(dfcu_raw, output_dir)
        
        # Create synchronized plot with missing value indicators
        create_synchronized_missing_value_plot(dfcu_raw, output_dir)

        # Original plots (with interpolated data) - keep existing functionality
        ticker_columns = [col for col in dfcu_tickers.columns if col != 'Date']
        ...
        fig, axes = plt.subplots(rows, cols, figsize=(6 * cols, 4.5 * rows))
        fig.suptitle('Copper (Interpolated Data)', fontsize=20, fontweight='bold')
        ...
        for i, ticker in enumerate(ticker_columns):
            ax.plot(dfcu_tickers['Date'], dfcu_tickers[ticker], linewidth=2, label=ticker)
        ax.set_title('All Copper Price Series Comparison (Interpolated)', fontsize=18, fontweight='bold')
        ax.set_xlabel('Date', fontsize=14)
        ax.set_ylabel('Price', fontsize=14)
        ...
        print(f"All copper plots saved in: {plots_dir}")
