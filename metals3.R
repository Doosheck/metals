...
#run_ml_pipeline_robust <- function(metal_name, col_name, df_data) {
  ...
  if (is.null(df_ml) || sum(df_ml$Target_Bubble) < 10) {
    message(paste("Skipping", metal_name, "- Not enough data/bubbles."))
    return(NULL)
  }
  ...
  # C. SHAP Calculation (Fast Mode)
  X_explain <- select(df_ml, -Date, -Target_Bubble)
  bg_X <- X_explain[sample(nrow(X_explain), 25), ] # Speed optimization
  rows_to_explain <- sample(nrow(X_explain), 100)
  
  shap_values <- kernelshap(fit_rf, X_explain[rows_to_explain, ], bg_X = bg_X)
  viz <- shapviz(shap_values)
  
  # --- FIX: Robust Class Extraction ---
  # Check if 'viz' contains multiple classes (mshapviz) or just one (shapviz)
  if (inherits(viz, "mshapviz")) {
    # It has multiple classes (usually Class 0 and Class 1).
    # We try to grab the one named "1" (Bubble), or default to the 2nd one.
    viz_bubble <- tryCatch({ viz[["1"]] }, error = function(e) viz[[2]])
  } else {
    # It is already a single object (rare, but possible)
    viz_bubble <- viz
  }
  
  # D. Plot
  
  # 1. Define a helper to clean the ugly names
  clean_labels <- function(x) {
    x <- gsub("_Lag1", "", x)          # Remove Lag1
    x <- gsub("_Level", "", x)         # Remove Level
    x <- gsub("_Ret", " Returns", x)   # Make Returns readable
    x <- gsub("_Vol", " Volatility", x)# Make Volatility readable
    x <- gsub("_dummy", " Bubble", x)  # For peer metals
    return(x)
  }
  
  # 2. Create the plot with bigger fonts and no title
  plot_obj <- sv_importance(viz_bubble, kind = "beeswarm") +
    theme_minimal() +
    labs(title = NULL) +  # REMOVE TITLE
    scale_y_discrete(labels = clean_labels) + # APPLY CLEAN LABELS
    theme(
      axis.text.y  = element_text(size = 14, color = "black"), # Left Variables (Big)
      axis.text.x  = element_text(size = 12, color = "black"), # Bottom Numbers (Big)
      axis.title.x = element_text(size = 14, face = "bold"),   # "SHAP value" Title (Big & Bold)
      legend.text  = element_text(size = 10),
      legend.title = element_text(size = 12),
      panel.grid.major.y = element_line(linetype = "dotted", color = "grey80") # Optional: nicer grid
    )
  
  return(list(model = fit_rf, plot = plot_obj, shap_obj = viz_bubble))
}
#run_ml_pipeline_old1 <- function(metal_name, col_name, df_data) {
  
  cat(paste("\n=== Processing:", metal_name, "===\n"))
  
  # 1. Feature Engineering
  # We wrap this in tryCatch to handle potential missing columns safely
  df_ml <- tryCatch({
    prepare_ml_data(metal_name, col_name, df_data)
  }, error = function(e) {
    message(paste("Error preparing data for", metal_name, ":", e$message))
    return(NULL)
  })
  
  if (is.null(df_ml)) return(NULL)
  
  # 2. Check for sufficient bubble instances
  # If there are fewer than 10 bubble days, the model cannot learn.
  if (sum(df_ml$Target_Bubble) < 10) {
    message("Skipping: Too few bubble episodes (<10) for valid training.")
    return(NULL)
  }
  
  # 3. Train Random Forest
  # probability = TRUE is required for SHAP with classification
  fit_rf <- ranger(
    as.factor(Target_Bubble) ~ ., 
    data = select(df_ml, -Date), 
    importance = "permutation",
  ...
  cat("Calculating SHAP values (Optimized)... \n")
  
  X_explain <- select(df_ml, -Date, -Target_Bubble)
  
  # Optimization: Use a smaller background sample (25) to speed up calculation.
  # This reduces time significantly while maintaining accuracy.
  bg_X <- X_explain[sample(nrow(X_explain), 25), ] 
  
  # Select rows to explain (100 random observations)
  rows_to_explain <- sample(nrow(X_explain), 100)
  
  # Run KernelSHAP
  shap_values <- kernelshap(fit_rf, X_explain[rows_to_explain, ], bg_X = bg_X)
  
  # 5. Filter for "Bubble" Class Only
  # shapviz returns values for Class 0 (No Bubble) and Class 1 (Bubble).
  # We only want to visualize drivers of Class 1.
  viz <- shapviz(shap_values)
  
  # Try to select the column named "1" or the second column
  viz_bubble <- tryCatch({
    viz[, "1"] 
  }, error = function(e) {
    viz[, 2] # Fallback to 2nd column if name is missing
  })
  
  # 6. Generate Plot
  plot_obj <- sv_importance(viz_bubble, kind = "beeswarm") +
    ggtitle(paste("Drivers of", metal_name, "Bubbles")) +
    theme_minimal() +
    theme(plot.title = element_text(face = "bold", hjust = 0.5))
  
  # 7. Return Everything
  # We return the raw 'shap_obj' so you can re-plot later without re-calculating!
  return(list(
    model = fit_rf, 
    plot = plot_obj, 
    shap_obj = viz_bubble, 
...
#run_ml_pipeline_old <- function(metal_name, col_name, df_data) {
  
  cat(paste("\n=== Processing:", metal_name, "===\n"))
  
  # A. Prepare Data
  # We catch errors in case a specific metal has missing columns
  tryCatch({
    df_ml <- prepare_ml_data(metal_name, col_name, df_data)
  }, error = function(e) {
    message(paste("Skipping", metal_name, ":", e$message))
    return(NULL)
  })
  
  if (!exists("df_ml")) return(NULL)
  
  # Check Class Balance
  # If there are fewer than 10 bubble days, the model cannot learn effectively.
  bubble_count <- sum(df_ml$Target_Bubble)
  if (bubble_count < 10) {
    message(paste("Skipping", metal_name, "- Too few bubble episodes (", bubble_count, ") for training."))
    return(NULL)
  }
  
  cat(paste("Training Random Forest on", nrow(df_ml), "obs with", bubble_count, "bubble days...\n"))
  
  # B. Train Random Forest (ranger)
  # importance = "permutation" is slower but more accurate for correlated features
  fit_rf <- ranger(
    as.factor(Target_Bubble) ~ ., 
    data = select(df_ml, -Date), 
    importance = "permutation",
  ...
  # Print OOB (Out-of-Bag) Error to console
  print(fit_rf) 
  
  # C. Explainability (SHAP)
  cat("Calculating SHAP values (this may take a moment)...\n")
  
  X_explain <- select(df_ml, -Date, -Target_Bubble)
  
  # Background Dataset: A random sample of 50 observations to serve as the baseline
  # This speeds up calculation significantly compared to using the whole dataset
  bg_X <- X_explain[sample(nrow(X_explain), 50), ]
  
  # Rows to Explain: A mix of 100 observations (prioritizing bubble periods if possible)
  # This gives us a representative view of what drives the bubbles.
  rows_to_explain <- sample(nrow(X_explain), 100)
  
  # Calculate SHAP using KernelSHAP (Model Agnostic)
  shap_values <- kernelshap(fit_rf, X_explain[rows_to_explain, ], bg_X = bg_X)
  # alternative: increase number of permutations (default n_vars * 2^11) - TIME!!!
  # shap_values <- kernelshap(fit_rf, X_explain[rows_to_explain, ], bg_X = bg_X, n_nsamples = 2000)
  
  # Create Visualization Object
  viz <- shapviz(shap_values)
  viz <- viz[, 2]  #only class 1=yes, there is the bubble
  
  # Generate Beeswarm Plot
  plot_obj <- sv_importance(viz, kind = "beeswarm") +
    ggtitle(paste("SHAP Drivers of", metal_name, "Bubbles")) +
    theme_minimal() +
    theme(plot.title = element_text(face = "bold", hjust = 0.5))
  
  return(list(model = fit_rf, plot = plot_obj, data = df_ml))
}
...
# ---Cleanup ---
plan(sequential)
saveRDS(ml_results, "ml_results_complete.rds")
